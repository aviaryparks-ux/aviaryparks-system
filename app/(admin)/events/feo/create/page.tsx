...
export default function CreateFEOPage() {
  ...
  const [formData, setFormData] = useState<AppEvent>({
    ...
    feoData: {
      ...
      additionalChargePax: 0,
      additionalChargePrice: 0,
      additionalChargeTotal: 0,
      grandTotal: 0,
      downPayment: 0,
      balancePayment: 0,
      zonaMorningTour: "Tolong disiapkan untuk 2 educator standby di ticketing 8.40",
      lunchArea: "",
      mealBoxKids: "Package B kids ( 2pcs chicken roll, 2pcs egg roll)",
      mealBoxKidsPax: 0,
      mealBoxTeacher: "Package A adults (ayam geprek + sambal matah)",
      mealBoxTeacherPax: 0,
      driver: "",
      notesCurator: "",
      notesTicketing: "Tolong disiapkan untuk tiket di jam 8.40",
      notesFBKitchen: "Tolong disiapkan untuk lunch box di jam 10.30",
      notesFBService: "Tolong di setup ruangan VN C untuk pax",
      notesHousekeeping: "Tolong pastikan semua area bersih",
      notesSecurity: "Tolong arahkan bis ke area ticketing",
      notesSalesMarketing: "Mohon untuk dilakukan dokumentasi",
    ...
    if (field.startsWith("feo.")) {
      ...
      setFormData(prev => {
        ...
        if (["additionalChargePax", "additionalChargePrice"].includes(feoField)) {
           ...
           const packageTotal = newFeoData.priceDetails.reduce((sum, item) => sum + item.total, 0);
  ...
  const handlePriceDetailChange = (index: number, field: string, value: any) => {
    ...
    const grandTotal = packageTotal + Number(formData.feoData!.additionalChargeTotal);
    ...
    setFormData(prev => ({
      ...
      feoData: {
        ...
        balancePayment: grandTotal - Number(prev.feoData!.downPayment)
      }
    }));
  };
  ...
  const checkClash = async () => {
    try {
      ...
      const isClashing = snapshot.docs.some(doc => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const counterRef = doc(db, "counters", "feo");
      ...
      toast.success("FEO Berhasil Dibuat");
      router.push("/events/feo");
    } catch (error: any) {
      setErrorMsg(error.message);
      toast.error("Gagal membuat FEO");
    } finally {
      setLoading(false);
    }
  };
    ...
    <ProtectedRoute requiredFeature="manage_feo">
      <div className="w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-32">
        <div className="mb-8">
          ...
          <p className="text-slate-500 mt-1">Isi formulir di bawah ini untuk membuat event fieldtrip baru.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-sm border border-slate-100 lg:col-span-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-2">Nama Sekolah / Grup / Instansi *</label>
                  <input type="text" required list="clients-list" value={formData.feoData?.schoolName} onChange={(e) => {
                    handleChange("feo.schoolName", e.target.value);
                    const selectedClient = clients.find(c => c.name === e.target.value);
                    if (selectedClient) {
                      if (selectedClient.address) handleChange("feo.address", selectedClient.address);
                    }
                  }} className="w-full border border-slate-300 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none" placeholder="Ketik atau pilih dari database..." />
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
                  ...
                  <div>
                    ...
                    <input type="number" readOnly value={formData.feoData?.paxTotal} className="w-full border border-emerald-300 bg-emerald-50 rounded-lg px-3 py-2 text-sm font-bold" />
                  </div>
                </div>
              </div>
            </div>
                ...
                <table className="w-full border-collapse min-w-[600px]">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td colSpan={2} className="p-3 border border-slate-200">
                        <input type="text" value={formData.feoData?.downPayment === 0 ? "" : formData.feoData?.downPayment.toLocaleString('id-ID')} onChange={(e) => handleCurrencyChange("feo.downPayment", e.target.value)} placeholder="0" className="w-full p-2 border rounded-lg font-bold" />
                      </td>
                    </tr>
                    <tr>
          ...
          <div className="flex justify-end gap-4 mt-8">
            ...
            <button type="submit" disabled={loading} className="px-8 py-2.5 rounded-xl font-bold text-white bg-emerald-600 hover:bg-emerald-700 transition-colors shadow-lg shadow-emerald-500/20 disabled:opacity-50">
              {loading ? "Menyimpan..." : "Simpan & Lanjutkan FEO"}
            </button>
          </div>
        </form>
      </div>
