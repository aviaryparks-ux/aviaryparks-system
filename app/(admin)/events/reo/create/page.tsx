...
import { collection, addDoc, serverTimestamp, query, where, getDocs, getDoc, doc, orderBy, runTransaction } from "firebase/firestore";
...
export default function CreateREOPage() {
  ...
  const handleDPChange = (dp: number) => {
    setFormData(prev => ({
      ...prev,
      reoData: {
        ...prev.reoData!,
        downPayment: dp,
        balancePayment: prev.reoData!.grandTotal - dp
      }
    }));
  };

  const checkClash = async () => {
    if (!formData.startDate || !formData.startTime || !formData.endTime || !formData.reoData?.restaurantName || !formData.reoData?.venueSection) return false;

    const q = query(
      collection(db, "events"),
      where("startDate", "==", formData.startDate),
      where("reoData.restaurantName", "==", formData.reoData.restaurantName),
      where("reoData.venueSection", "==", formData.reoData.venueSection)
    );
    ...
    snapshot.forEach((doc) => {
      ...
      if (event.status !== 'cancelled' && event.startTime && event.endTime) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isClashing) {
        setErrorMsg(`Jadwal BENTROK! Sudah ada event lain di ${formData.reoData?.restaurantName} (${formData.reoData?.venueSection}) pada jam tersebut.`);
      ...
      if (!creatorSignatureUrl) {
        setErrorMsg("Anda belum memiliki Tanda Tangan Digital. Harap atur di menu Profil terlebih dahulu.");
        toast.error("Tanda tangan belum diatur");
        setLoading(false);
        return;
      }
      ...
      const counterRef = doc(db, "counters", "reo");
      await runTransaction(db, async (transaction) => {
        const counterDoc = await transaction.get(counterRef);
        let nextSequence = 1;
        if (counterDoc.exists()) {
          nextSequence = (counterDoc.data().sequence || 0) + 1;
        }
        transaction.set(counterRef, { sequence: nextSequence }, { merge: true });

        const newEventRef = doc(collection(db, "events"));
        transaction.set(newEventRef, { ...docData, documentNumber: nextSequence });
      });
      toast.success("REO berhasil dibuat!");
      router.push("/events/reo");
    } catch (error: any) {
      console.error("Error creating REO:", error);
      toast.error("Gagal membuat REO: " + error.message);
    } finally {
      setLoading(false);
    }
  };
    ...
    <ProtectedRoute requiredFeature="manage_reo">
      <form onSubmit={handleSubmit} className="w-full max-w-7xl mx-auto space-y-8 px-4 sm:px-6 lg:px-8 py-8 pb-32">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h1 className="text-2xl font-black text-slate-800 tracking-tight">Buat REO Baru</h1>
            <p className="text-slate-500 mt-1">Isi formulir Restaurant Event Order</p>
          </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    placeholder="Contoh: Ulang Tahun Bpk Budi"
                  />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input type="text" value={formData.reoData?.eventType} onChange={(e) => handleChange("reo.eventType", e.target.value)} className="w-full border border-slate-300 rounded-xl px-4 py-2.5" placeholder="Social Event" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input type="text" list="clients-list" value={formData.reoData?.companyName} onChange={(e) => {
                    handleChange("reo.companyName", e.target.value);
                    const selectedClient = clients.find(c => c.name === e.target.value);
                    if (selectedClient) {
                      if (selectedClient.address) handleChange("reo.address", selectedClient.address);
                      if (selectedClient.phone) handleChange("reo.phone", selectedClient.phone);
                    }
                  }} className="w-full border border-slate-300 rounded-xl px-4 py-2.5" placeholder="Ketik atau pilih dari database..." />
                ...
                <div>
                  ...
                  <input type="text" value={formData.reoData?.typeOfEvent} onChange={(e) => handleChange("reo.typeOfEvent", e.target.value)} className="w-full border border-slate-300 rounded-xl px-4 py-2.5" placeholder="Birthday" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input type="number" value={formData.reoData?.pax === 0 ? "" : formData.reoData?.pax} onChange={(e) => handleNumberChange("reo.pax", e)} placeholder="0" className="w-full border border-slate-300 rounded-xl px-4 py-2.5" />
                </div>
                <div>
                  ...
                  <tbody>
                    {formData.reoData?.priceDetails.map((detail, idx) => (
                      <tr key={idx} className="border-b border-slate-100">
                        <td className="p-2"><input type="text" value={detail.packageName} onChange={(e) => handlePriceDetailChange(idx, 'packageName', e.target.value)} className="w-full p-2 border border-slate-200 rounded" placeholder="Nama Paket / Item" /></td>
                        <td className="p-2"><input type="number" value={detail.pax === 0 ? "" : detail.pax} onChange={(e) => handlePriceNumberChange(idx, 'pax', e)} className="w-full p-2 border border-slate-200 rounded" placeholder="0" /></td>
                        <td className="p-2">
                          <div className="flex items-center">
                            <span className="text-slate-500 mr-2">Rp</span>
                            <input type="text" value={detail.price === 0 ? "" : detail.price.toLocaleString('id-ID')} onChange={(e) => handlePriceCurrencyChange(idx, 'price', e.target.value)} className="w-full p-2 border border-slate-200 rounded" placeholder="0" />
                          </div>
                        </td>
