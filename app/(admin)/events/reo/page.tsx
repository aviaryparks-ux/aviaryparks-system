...
export default function REOListPage() {
  const { user } = useAuth();
  ...
  return (
    <ProtectedRoute requiredFeature="view_reo">
      <div className="w-full space-y-6 px-4 sm:px-6 lg:px-8 py-8 pb-32">
        {/* Header */}
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          <div>
            <h1 className="text-2xl font-black text-slate-800 tracking-tight">Restaurant Event Order (REO)</h1>
            <p className="text-slate-500 mt-1">Kelola pesanan event dan banquet restoran</p>
          </div>
          <Link
            href="/events/reo/create"
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/50 border-b border-slate-100 text-slate-500 text-sm">
                  ...
                  <th className="p-4 font-semibold">NAMA KLIEN & EVENT</th>
                  <th className="p-4 font-semibold">SALES INCHARGE</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={6} className="p-8 text-center text-slate-400">Belum ada data REO</td>
                  </tr>
                ) : (
                  filteredEvents.map((event) => (
                    <tr key={event.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="p-4 text-sm font-bold text-slate-800">
                        {event.reoData?.guestName || event.clientName || "-"}
                        <div className="text-xs font-normal text-slate-500 mt-0.5">{event.title}</div>
                      </td>
                      <td className="p-4 text-sm font-medium text-slate-600">
                        {event.reoData?.salesIncharge || "-"}
                      </td>
                      <td className="p-4 text-sm text-slate-600">
                        {event.reoData?.pax || 0} pax <br />
                        <span className="text-xs text-slate-400">{event.reoData?.restaurantName || "-"} - {event.reoData?.venueSection || "-"}</span>
                      </td>
                      <td className="p-4">
