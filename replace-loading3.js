...
walkDir('./app/(admin)', function(filePath) {
  if (filePath.endsWith('page.tsx') || filePath.endsWith('layout.tsx')) {
    ...
    // Pattern for: if (loading) { return ( <div...> ... animate-spin ... </div> ); }
    // Using non-greedy match .*?
    content = content.replace(
      /if\s*\(\s*loading\s*\)\s*\{\s*return\s*\(\s*<div[\s\S]*?animate-spin[\s\S]*?<\/div>\s*\);\s*\}/g,
      'if (loading) return <LoadingScreen fullScreen={false} size={150} />;'
    );

    // Pattern for: if (loading) return ( <div...> ... animate-spin ... </div> );
    content = content.replace(
      /if\s*\(\s*loading\s*\)\s*return\s*\(\s*<div[\s\S]*?animate-spin[\s\S]*?<\/div>\s*\);/g,
      'if (loading) return <LoadingScreen fullScreen={false} size={150} />;'
    );
